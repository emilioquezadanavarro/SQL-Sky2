...
QUERY_DELAYED_FLIGHTS_BY_AIRPORT = 'SELECT flights.*, flights.ID as FLIGHT_ID, airlines.airline AS AIRLINE, flights.DEPARTURE_DELAY AS DELAY FROM flights JOIN airlines ON flights.airline = airlines.id WHERE (flights.DEPARTURE_DELAY > 0 OR flights.ARRIVAL_DELAY >0) AND flights.ORIGIN_AIRPORT = :code'
